...
def CREATE_DADAFRAMES_v2(gameHistoryFolderPath, countryThemeFolder, Non_Odds_cols, num_bookies=5, oddsGroupsToExclude = [], preferedOrder = [], preferedBackupBookies = [], train_mode=True, skip=False):
        ...
        # Implement preferedBackupBookies
        preferedBackupBookies = [og for og in preferedBackupBookies if og not in standard_ogs_ordered]
        backup_ogs_ordered = [og for og in backup_ogs_ordered if og not in preferedBackupBookies]
        backup_ogs_ordered = preferedBackupBookies + backup_ogs_ordered
        ...
        for (filename, (df, non_ogs, ogs)) in filename_dict.items():
                ...
                # ogs_to_rename = list(rename_plan.keys())
                # if ogs_to_rename.count('BF') > 0:
                #        debug = 3
                ...
                filename_dict[filename] = (df, non_ogs, ogs, rename_plan)        # keep rename_plan for later later maintainance use.
                cols = df.columns
                if common_cols is None: common_cols = cols
                else: common_cols = [col for col in cols if col in common_cols]
        ...
        # re-create index, to make sure indices are unique.
        df_built.index = pd.RangeIndex(len(df_built.index))
        df_built.index = range(len(df_built.index))
        ...
        # Debug: df_built[df_built['Date']==datetime.date('4/14/2025')][['Div', 'HomeTeam', 'AwayTeam', 'HDA3H', 'HDA3D', 'HDA3A']]
        df_built = improve_uk_dataframe(df_built, dropNa=False)
        # Debug: df_built[df_built['Date']==datetime.date(2025,4,14)][['Div', 'HomeTeam', 'AwayTeam', 'HDA3H', 'HDA3D', 'HDA3A']]
                ...
                if dateOnly == False:
                        ...
                        for index, row in df.iterrows():
                                ...
                                for og in ogs:  # replace keeping std and mean.
                                        if og == nn_og: continue
                                        if pd.isnull(row[og+'H']) or pd.isnull(row[og+'D']) or pd.isnull(row[og+'A']):
                                                bookies_with_missing_odds.append(og)
                                                df.at[index, og+'H'] = row[nn_og+'H']; df.at[index, og+'D'] = row[nn_og+'D']; df.at[index, og+'A'] = row[nn_og+'A']
                                                # row[og+'H'] = row[nn_og+'H']; row[og+'D'] = row[nn_og+'D']; row[og+'A'] = row[nn_og+'A']
                                        # col = og+'H'
